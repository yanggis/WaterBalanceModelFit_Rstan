...
data {
	...
	vector[N]			ETi;						// observed "instant" PET
...
parameters { 	// PARAMETERS ARE ASSIGNED UNIFORM PROB ON VALID INTERVAL UNLESS SPECIFIED OTHERWISE IN MODEL
	...
	real<lower=0.75,upper=1.25>							c;			// ETi coefficient 
}

model {
	real 			mu_nu;							// temp value of mu_nu
	real 			Pr_chi[2];					// temp var for log component densities
	real 			S1;									// temp var derived each MCMC iteration
	real 			K2;									// temp var derived each MCMC iteration
	real 			chix[N];						// temp var for chi - c*ETi
	...
	for (t in 1:N){
		...
		for (g in 1:2) {
			Pr_chi[g] <- log(chi_theta[g]) + normal_log(chi[t], chi_mu[g], chi_sig[g]);
		}
		increment_log_prob(log_sum_exp(Pr_chi));
		
		chix[t] <- chi[t] - c * ETi[t];
		// modeled nu: inv_logit() is analytic approx to Heaviside fcns
		mu_nu <- ( chix[t]-K1 ) *S1 *inv_logit( A*(chix[t]-K1) ) + (chix[t]-K2) *(S2-S1) *inv_logit( A*(chix[t]-K2) );
